...
// ============================================================================
//
// Vendored from supertone-inc/supertonic (rust/src/helper.rs), MIT-licensed
// sample code. Faithful Rust port of the Supertonic-3 ONNX pipeline (text
// front-end + 4-model chain + WAV writer). Treated as an internal detail of
// speak-core; the public SDK lives in lib.rs. Keep in sync with upstream
...
/// Load TTS components
/// Directory where the CoreML EP caches its compiled model, so the CLI does
/// not recompile the ONNX graph to CoreML on every invocation. Sits next to
/// the `onnx/` directory.
fn coreml_cache_dir(onnx_dir: &str) -> String {
    let onnx = Path::new(onnx_dir);
    let base = onnx.parent().unwrap_or(onnx);
    let cache = base.join("coreml_cache");
    let _ = std::fs::create_dir_all(&cache);
    cache.to_string_lossy().into_owned()
}

/// Build one ONNX session. With `use_gpu` and the `coreml` feature, the CoreML
/// execution provider is registered; ONNX Runtime falls back to CPU silently
/// for unsupported ops, or entirely if CoreML cannot initialize.
fn build_session(model_path: &str, cache_dir: &str, use_gpu: bool) -> Result<Session> {
    ...
    if use_gpu {
        ...
        // Stringify the error: its payload is the non-Send SessionBuilder, so
        // it cannot convert into anyhow::Error directly.
        builder = builder
...
pub fn load_text_to_speech(onnx_dir: &str, use_gpu: bool) -> Result<TextToSpeech> {
    ...
    let cache_dir = coreml_cache_dir(onnx_dir);
    let dp_ort = build_session(&dp_path, &cache_dir, use_gpu)?;
    let text_enc_ort = build_session(&text_enc_path, &cache_dir, use_gpu)?;
    let vector_est_ort = build_session(&vector_est_path, &cache_dir, use_gpu)?;
    let vocoder_ort = build_session(&vocoder_path, &cache_dir, use_gpu)?;
